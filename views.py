...
def producer_dashboard_view(request):
    ...
    context = {'greetings': greetings, 'payments': payments, 'collections': collections, 'balance': account.balance, 'subscription': account.montly}
...
def request_pickup_view(request):
    ...
    if request.method == 'POST':
        ...
        # subtract 20 from account balance
...
def process_request_view(request, id):
    ...
    if request.method == 'POST':
        ...
        # add 20 to collector account balance
        collector_account.balance = account.balance + 20
